...
The analysis reveals a statistically significant difference in crash frequency across seasons. The Chi-Square test yields a p-value well below the 0.05 significance threshold, leading us to reject the null.
...
While the Chi-Square test examined aggregate totals, it did not account for year-to-year variability. To address this, we aggregate crash counts by year and season to calculate the mean yearly crash rate. We then perform a One-Way ANOVA to determine if the average number of airplane crashes per year differs significantly across the four seasons.

The ANOVA results yield a p-value of 0.039, which is below the 0.05 significance level. 
...
No significant evidence suggest that there are monthly differences of fatality rate.
...
To describe the historical trend of aviation safety, we aggregate the total number of crashes per year. We then fit a simple linear regression model to determine if there is a statistically significant linear increase or decrease in crash frequency over the recorded time period (1908–2009).

The linear regression analysis reveals a statistically significant upward trend.
```{r}
...
While the simple linear regression suggests a significant upward trend, visual inspection implies a non-linear pattern with distinct peaks and valleys. To scientifically determine the best approach for describing this historical data, we performed Cross-Validation (CV). We compared three candidate models: a linear model, a piecewise linear model (assuming a structural break around 1990), and a generalized additive model (GAM) using smoothing splines. We evaluated them based on their ability to predict unseen data subsets.
...
The visual comparison of the cross-validated predictions clearly demonstrates the limitations of the linear model. The linear fit (left panel) fails to capture the mid-century rise and the late-century decline in crashes, resulting in high systematic error. The piecewise model (middle panel) offers an improvement by accounting for the recent downtrend, but the Smooth (GAM) model (right panel) provides the superior fit。
