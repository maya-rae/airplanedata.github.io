...
## Interactive Filters + Map + Table

```{r, echo=FALSE}
# Prepare Crosstalk dataset
df_display <- df_map |>
  mutate(
    year = as.factor(year),
    operator = as.factor(operator)
...
# Build Filter Sidebar + Map + Table
bscols(
  widths = c(3, 9),
...
```{r}
# Identify top 5 operators by number of crashes
top_ops <- df_map |>
...
# Color palette for the 5 operators
pal <- colorFactor(
